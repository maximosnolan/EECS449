...
class Doorman:
    ...
    def handleRequest(self, user_speech: str) -> str:
        ...
        if (time.time() - self.last_visitor_time < self.VISITOR_TIME_DELAY_ALLOWED or self.v_data == None):
            ...
            # call call qdrant
            results = search(self.qdrant_client, face_embeddings)
            if len(results) == 1:
                self.v_data = results[0].payload

        intent_id = self._get_intent(user_speech)
        text_response = self._get_response(intent_id)
